...
public class MyBirthday {
	 ...
	 * 2. 값을 조회만 할 수 있고 변경은 불가능 하기때문에 직접조회보다 보안성을 높여줍니다.
	...
	// year, month에 대해서도 setter/getter를 모두 만들어보자.
	public void setYear(int year) {
		if(year > 2023) { //if(year < 1900 || year > 2023) {
			this.year = 2023;
		}else if(year < 0) {
			this.year = -year;
		}else {
	...
	public void setMonth(int month) {
		if(month < 1 || month > 12) { //if(month > 12)도 가능
			this.month = month % 12;
		}else if(month < 1) {
			this.month = (-month % 12);
		}else {
